...
#from slam3r.models_test import Image2PointsModel, Local2WorldModel, inf
...
#                     break
#                 else:
#                     print(f"云端跟丢帧序列文件夹为空，等待 {check_interval} 秒后重试...")
#             else:
...
#             print("用户终止检测")
#             break
def wait_until_file_exists(target_dir, filename="status.txt", check_interval=5):
    """持续检测目标目录下是否存在指定txt文件"""
    target_file = os.path.join(target_dir, filename)  # 构造完整文件路径[3,4](@ref)
    ...
    while True:
        try:
            ...
            if os.path.exists(target_dir) and os.path.isdir(target_dir):
                # 检测文件存在性[2,4](@ref)
                if os.path.isfile(target_file):  # 精确判断是否为文件[1,4](@ref)
                    print(f"检测到 {target_file} 存在，跳出循环")
                    return True
                else:
                    print(f"文件 {filename} 不存在，等待 {check_interval} 秒后重试...")
            else:
                print(f"路径 {target_dir} 无效或非目录")
            
            time.sleep(check_interval)  # 保持原有间隔检测[6,7](@ref)
...
if __name__ == "__main__":
    while True:
        save_dir1 = "/data/xjluo/SLAM3R/data/Replica_demo/data"
        wait_until_file_exists(save_dir1, filename="Image_data_isok.txt", check_interval=1)
        #wait_until_folder_non_empty(save_dir1, check_interval=1)
        stat_time = time.time()
        ...
        i2p_model = Image2PointsModel.from_pretrained('/data/xjluo/SLAM3R/model/slam3r_i2p_model')
        stat_time_2 = time.time()
        #l2w_model = Local2WorldModel.from_pretrained('siyan824/slam3r_l2w')
        l2w_model = Local2WorldModel.from_pretrained('/data/xjluo/SLAM3R/model/slam3r_l2w_model')
        # 启用TF32加速（需Ampere架构以上GPU）
        ...
        if args.dataset:
            ...
            dataset = DataLoader(dataset, batch_size=1, shuffle=False, num_workers=16, pin_memory=True, persistent_workers=True)
            print('Loading end----------')
        ...
        if os.path.exists(save_dir1):
            try:
                shutil.rmtree(save_dir1)  # 递归删除非空文件夹[3,6,7](@ref)
            except Exception as e:
                print(f"删除文件夹失败: {e}")
        os.makedirs(save_dir1, exist_ok=True)
        print("#" * 64)
        end_time = time.time()
        print('总耗时：', end_time - stat_time, '秒')
